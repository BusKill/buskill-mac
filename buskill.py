...
def Normal_Operation(Device, Trigger):
    os.chdir("/Applications/BusKill/Triggers/"+Trigger+"/")
...
def Get_Triggers():
    Triggers = []
    dirlist = os.listdir("/Applications/BusKill/Triggers/")
    for dir in dirlist:
        if os.path.isdir("/Applications/BusKill/Triggers/"+dir) == True:
            Triggers.append(dir)
    return Triggers
...
def Query_Trigger(Trigger):
    with open(Get_App_File_Path(sys.argv[0])+"/Applications/BusKill/Triggers/"+Trigger+"/TriggerInfo.txt") as InfoFile:
        DATA = InfoFile.read().splitlines()
        for line in DATA:
            print(line)

def Main(args):
    os.chdir("/Applications/BusKill/")
    Dev = False
    Trig = False
    Quer = False
    Query = False
    ...
    if Query == True:
        ...
        with open("/Applications/BusKill/Triggers/"+Trigger+"/TriggerInfo.txt") as InfoFile:
